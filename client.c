...
int main(int argc, char *argv[]) {
	...
	if (argc != 4) {
		printf("사용법 : %s sever_ip  port name  user name\n", argv[0]);
		exit(0);
	}
	...
	while (1) {
		...
		if (select(maxfdp1, &read_fds, NULL, NULL, NULL) < 0)
			errquit("select fail");
		if (FD_ISSET(s, &read_fds)) { // 서버로부터의 입력 
			int nbyte;
		...
		if (FD_ISSET(0, &read_fds)) { // 콘솔로 부터의 입력 
			if (fgets(bufmsg, MAXLINE, stdin)) {
				...
				if(strstr(bufmsg,"chatlog")!=NULL){//만약 chatlog를 받으려고 한다면
					...
					int recv_len=0;
					int totaln =0;
					int dest_fd = open("downloaded_chatlog.log",O_CREAT|O_WRONLY,__S_IWRITE|__S_IREAD);
					while(1){
					...
					close(dest_fd);
					printf("download chalog complete\n");
					fprintf(stderr, "\033[1;32m");	//글자색을 녹색으로 변경
					fprintf(stderr, "%s>", argv[3]);//내 닉네임 출력
					continue;
				}
